...
2. 仅从这些真实输入定稿 Fusion SLK Run 的初始 CELL 列表。
...
1. Supervisor 创建Fusion Checker可见对话，交付Fusion记录、CELL计划、冻结输入、合同和集成空间；初始化 Fusion SLK Run 时登记 `source_kind=clk` 与 `source_project_name=<CLK项目名>`。
...
准备完成后启动这一条Fusion Chain。Checker与Worker按SLK推进线性 CELL；共享Supervisor负责融合层协调及最终D2，不接管日常CELL派工或D1。

若真实融合暴露合同缺口，Supervisor记录证据并协调受影响的合同修订；产品目标、业务选择或Owner权限发生变化时，再向Owner提出一个清楚的问题。

## 完成后

当Fusion的全部 CELL 已完成D0和D1并准备接受最终D2时，调用`$clk-close-run`。
