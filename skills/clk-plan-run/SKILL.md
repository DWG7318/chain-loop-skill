...
4. 为每条Chain规划一个线性 SLK Run 和全部初始 CELL，说明交付成果及其与Fusion Chain的关系。
...
8. 写出Fusion Chain的目标、正式输入、预期完整系统和初步 CELL 轮廓。具体 CELL 及模型在真实Chain成果通过D2后定稿。
9. 汇总Chain、初始CELL、合同、隔离、检验和Fusion轮廓，交给Owner确认本次CLK Run及开工；确认后的结构内容交由Supervisor创建和修改`CLK-CHAIN-MAP.md`。
